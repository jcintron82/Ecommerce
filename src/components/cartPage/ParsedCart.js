...
//.MAP FUNCTION BELOW TAKES THE ADDED ITEMS IN THE USER CART AND ADDS THEM TO THE CART.
//CURRENTLY TRYING TO GET THE ORDER SUMMARY SECTIONS TO WORK
let userCartArr = [];

export function ParsedCart() {
let totalQty = 0.00;
let orderSubTotal = 0.00;
let orderTaxAmt = 0.00;
let total$ = 0.00;
        ...
        <div>
            {
                userCartArr.map((product, name) => (
                    ...
                    <div>{ product.orderQty } {changeData(product)}</div>
                    <div><select name='qty' className="cartQtyInput" type='number' placeholder={ product.orderQty }>
